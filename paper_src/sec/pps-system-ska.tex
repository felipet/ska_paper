...
An important feature of the system that must be taken into account is that the PPS pulse
...
\gutinote{cuando se dice "the former" obligatoriamente hay que hablar de "the latter". En caso contrario habría que reescribirlo quitando "the former"}
...
The WR-ZEN board is a proprietary design developed in 2015 by Seven Solutions S.L.. 
The major design changes regarding previous WR-nodes designs
...
\gutinote{figura no referenciada en texto}
...
described in \cite{Rizzi2016}. This contribution states that, in
...
microprocessor and the FPGA inside the Zynq SoC. \gutinote{el gateware es el que configura el ARM?????????}
The main block diagram is shown in Figure \ref{fig:gateware_first_level}. 
It includes the WRPC Dual port (WRPC-2p) core that implements the high accuracy timing protocol based on
...
\begin{figure}[H] \centering
	...
	project design. WR-ZEN FPGA gateware design. The the WRPC-2p core that is responsible for
...
and the kernel support.  The former includes the Zen library that implements the
