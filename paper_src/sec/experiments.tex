...
In this section, we detail the experiments performed to demonstrate that the WR-ZEN
is a suitable node for the SKA Telescope PPS distribution system and the results obtained. 
In order to evaluate properly the project's timing requirements, we have performed 
three experiments:

\begin{itemize}
	\item \textbf{A WR node comparison:} We have study the PPS stability of three different WR devices 
	previously analysed in section \textcolor{red}{añadir la seccion}.
...
\end{figure}
\gutinote{los caption de las figuras tienen que decir lo que es, lo que representa, no necesariamente evaluar su contenido, para eso ya está en texto}
...
\begin{figure}
    ...
    \label{fig:tdev_exp1}
\end{figure}
\gutinote{los caption de las figuras tienen que decir lo que es, lo que representa, no nocesariamente evaluar su contenido, para eso ya está en texto}
...
%	charact_zen}. \textcolor{red}{esto hay que explicarlo mÃ¡s e intentarlo 
%	meterlo dentro de su secciÃ³n}}
%	\label{tab:exp1res}
%\end{table*}
...
SKA timing system. In section \ref{sec:ska} we presented an estimation of the final 
nodes that would be needed to be synchronized for the SKA: around 250 endpoints that 
must be synchronized with an accuracy below 2 ns. A WR network with only two 
hops is suitable to synchronize up to 306 end-nodes by using BC WRSs.

We set up a test network composed of a WRS at the top of the hierarchy, which is locked to an 
external time reference, another WRS as intermediate BC and a WR-ZEN as an
...
\begin{figure}
	...
	\label{fig:mtienet}
\end{figure}
\gutinote{la fuente de esta figura sigue pequeña. Ponedla como las demás.}
...
The proposed system has been tested by JIVE \textcolor{red}{[REF]} in the SKA specific dessert zones of 
...
cable
\textit{round-trip} time (CRTT) and the PPS performance. To evaluate this, we 
have used a climate chamber in the laboratory and some fibre spools of tens of 
kilometers. Wind impact has not been evaluated properly with our equipment, \gutinote{no se ha evaluado correctamente, o es que no se ha evaluado¿?}
...
been measured for a temperature range from 20 ÂºC to 50 ÂºC with 10 ÂºC steps. 
...
\begin{table*}\centering
	...
	\begin{tabular}{@{} cccccc@{}}%\toprule
		...
		\textbf{Spool temp (ÂºC)} & $\overline{x}$ & $s$ & & $\overline{x}$ 
		& $s$ \\ \midrule
	...
	\caption{Results of the thermal characterization for an operational fibre 
		temperature in range 20ÂºC to 50ÂºC with 10ÂºC steps.}
	\label{tab:temp}
\end{table*}
...
operational temperature of a 50 km fibre link is set in a point from 20ÂºC to 
50ÂºC. The measurement of the rest of dependent values such as CRTT and PPS offset
...
3213 ps/ÂºC. It is important to remark the huge variation in the propagation delay. 
...
difference for the PPS offset is 211 ps which leaves us a 7 ps/ÂºC and if we 
divide by the total link length: $0.14 ps/ÂºC \cdot km$.
...
observed offset variation for a long distance link and a 30ÂºC temperature 
