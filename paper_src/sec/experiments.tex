...
The experiments that have been performed are briefly described in the following lines:

\klyonenote{Completar con más información y revisar el resto de la sección...}
\gutinote{yo pondría los experimentos con bullet points. Qué experimentos vamos a hacer y para demostrar qué. Ha de quedar claro.}

\begin{itemize}
	\item {WR-ZEN performance test: This scenarios connects two WR-ZEN boards and analyses
	the timing performance. The goal of this test is to demostrate that the platform is
	a valid candidate to be used as part of the PPS distribution system.}
	\item {Scalability test: It uses two WR switches and a WR-ZEN in a cascade configuration. The main purpose is to check that this kind of topology does not affect to the timing performance.}
	\item {Temperature influence test: It connects two WR-ZEN boards by a long fibre link which temperature is modified during the test. This test demostrates that these environment conditions do not reduce the timing performance of the system.}
\end{itemize}
...
\missingfigure{Jose Luis: Falta imagen del setup}
...
period. In addition we have repeated the same experiment for another WR node, the WR-LEN. \ftglnote{se debería haber detallado la LEN en el párrafo ese de comparativa así que aquí no menciono nada más.} The results ... \ftglnote{completar cuando estén terminados.}
