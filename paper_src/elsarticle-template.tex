...
The ARM microprocessor can be programmed with a Operating system as Linux or with a \textsc{bare-metal} application that manages the hardware resources directly. In the SKA system, the WR-ZEN is used with a Linux kernel to implement the specific applications needed by the system. So, a process must be defined to build the kernel and all software components (daemons, applications, etc). To easy this task, the Buildroot tool is used. It is responsible for generating a new Operating System based on Linux with multiple applications to get a fully operational environment. 
